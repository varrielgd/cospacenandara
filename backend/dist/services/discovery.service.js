...
class DiscoveryService {
     ...
     */
    static async discoverImporters(query, sessionId) {
        const foundImporterIds = [];
        try {
            index_1.logger.info(`Starting AI discovery for query: ${query}, session: ${sessionId}`);
            ...
            index_1.logger.info(`Found ${discoveredImporters.length} importers from AI discovery`);
            ...
            for (let i = 0; i < discoveredImporters.length; i++) {
                ...
                try {
                    ...
                    if (existing) {
                        index_1.logger.info(`Skipping existing importer: ${importer.companyName}`);
                        continue;
                    }
                    ...
                    await google_sheets_service_1.GoogleSheetsService.syncImporter(created).catch((err) => {
                        index_1.logger.error('Google Sheets sync failed:', err);
                    });
                    index_1.logger.info(`Successfully discovered: ${created.companyName}`);
                }
                catch (err) {
                    index_1.logger.warn(`Failed to process importer:`, err);
                }
            ...
            index_1.logger.info(`Discovery completed with ${foundImporterIds.length} new importers found`);
     ...
     */
    static async generateImportersFromQuery(query) {
        ...
        const templates = importerTemplates[country] || [
