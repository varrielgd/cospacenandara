...
const getAllSamples = async (_req, res) => {
    try {
        const samples = await index_js_1.prisma.sample.findMany({
        ...
        index_js_1.logger.error('Error fetching samples:', error);
...
const createSample = async (req, res) => {
    try {
        ...
        const sample = await index_js_1.prisma.sample.create({
            data: sampleData,
            include: { importer: { select: { companyName: true } } }
        });
        if (req.user) {
        ...
        index_js_1.logger.error('Error creating sample:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
...
const updateSample = async (req, res) => {
    try {
        ...
        const sampleData = req.body;
        const oldSample = await index_js_1.prisma.sample.findUnique({ where: { id: id } });
        if (!oldSample)
            return res.status(404).json({ message: 'Sample not found' });
        const sample = await index_js_1.prisma.sample.update({
            where: { id: id },
            data: sampleData,
            include: { importer: { select: { companyName: true } } }
        });
        ...
        index_js_1.logger.error('Error updating sample:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
...
const deleteSample = async (req, res) => {
    try {
        ...
        if (!id)
            return res.status(400).json({ message: 'ID is required' });
        await index_js_1.prisma.sample.delete({ where: { id: id } });
        return res.status(204).send();
    }
    catch (error) {
        index_js_1.logger.error('Error deleting sample:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
