...
const getAllSuppliers = async (req, res) => {
    try {
        ...
        index_1.logger.error('Error fetching suppliers:', error);
...
const getSupplierById = async (req, res) => {
    try {
        ...
        index_1.logger.error('Error fetching supplier:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
...
const createSupplier = async (req, res) => {
    try {
        ...
        index_1.logger.error('Error creating supplier:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
...
const updateSupplier = async (req, res) => {
    try {
        ...
        index_1.logger.error('Error updating supplier:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
...
const deleteSupplier = async (req, res) => {
    try {
        ...
        if (!id)
            return res.status(400).json({ message: 'ID is required' });
        await index_1.prisma.supplier.delete({ where: { id: id } });
        return res.status(204).send();
    }
    catch (error) {
        index_1.logger.error('Error deleting supplier:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
...
const importSuppliers = async (req, res) => {
    try {
        ...
        for (const row of data) {
            ...
            if (!existing) {
                const created = await index_1.prisma.supplier.create({
                    data: {
                        companyName: row.companyName || row.CompanyName || row.company_name || '',
        ...
        index_1.logger.error('Error importing suppliers:', error);
        return res.status(500).json({ message: 'Internal server error' });
    }
};
