...
export class DiscoveryService {
    ...
    try {
      ...
      while (discoveredImporters.length < targetCount && attempts < maxAttempts) {
        ...
        try {
          ...
          const validatedBatch = batchArray.filter((imp: any) => {
  ...
  const countryMatch =
    impCountry.includes(requestedCountry) ||
...
// Filter duplicates
const newUniqueLeads = validatedBatch.filter((newLead: any) =>
  !discoveredImporters.some(
    (existing: any) =>
      existing.companyName.toLowerCase() ===
      newLead.companyName.toLowerCase()
  )
);
          logger.info(`Batch ${attempts}: Found ${validatedBatch.length} total, ${newUniqueLeads.length} unique new leads`);
          ...
          for (const lead of newUniqueLeads) {
            try {
              ...
              if (existing) {
                ...
                logger.info(`Lead already exists: ${lead.companyName} (${importerId})`);
              } else {
                ...
                logger.info(`Created new lead: ${lead.companyName} (${importerId})`);
          ...
          if (sessionId) {
            logger.info(`Updating session ${sessionId}: ${foundImporterIds.length} importers total`);
